...
class Retriever:
        ...
        with torch.no_grad():
            for k, q in enumerate(queries):
                ...
                if (
                    ...
                    encoded_batch = {
                        k: v.cuda() for k, v in encoded_batch.items()
                    }
    ...
    def search_document(self, query, top_n=10):
        ...
        # Removed exit(0) to allow the retrieval process to continue
...
def load_data(data_path):
    if data_path.endswith(".json"):
        ...
        raise ValueError(
            "Unsupported file format for queries. Use .json or .jsonl."
        )
    return data
...
def main(args):
    ...
    queries_data = load_data(args.query)

    # Extract queries based on your JSON structure
    # Adjust the key based on your actual JSON structure
    # For example, if each entry has a "question" field:
    queries = [entry["question"] for entry in queries_data]
    answers = [entry["answers"] for entry in queries_data]
    # Prepare a list to hold retrieval results
    retrieval_results = []

    for idx, (query, ans) in enumerate(zip(queries, answers)):
        print(f"Processing Query {idx + 1}/{len(queries)}: {query}")
        results = retriever.search_document(query, args.n_docs)
        retrieval_results.append(
            {"question": query, "answers": ans, "ctxs": results}
        )
    ...
    with open(output_path, "w") as fout:
        json.dump(retrieval_results, fout, indent=2)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--query",
        type=str,
        required=True,  # Made required to ensure it's provided
        help=".json or .jsonl file containing a list of questions",
    )
    parser.add_argument(
        "--passages",
        type=str,
    ...
    parser.add_argument(
        "--no_fp16", action="store_true", help="Inference in fp32"
    )
    parser.add_argument(
        "--question_maxlength",
        type=int,
    ...
    parser.add_argument(
        "--normalize_text", action="store_true", help="Normalize text"
    )
