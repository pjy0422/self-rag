...
import spacy
import torch
from metrics import accuracy, match
...
def call_model_rerank_w_scores_batch(
    ...
    if do_retrieve is True:
        evidence_augmented_inputs = [
            prompt
            + "[Retrieval]<paragraph>{0}</paragraph>".format(para["text"])
...
def main():
    ...
    parser.add_argument(
        ...
        default="/home/guest-pjy/.cache/huggingface/hub",
    )
    parser.add_argument(
        "--ndocs",
        type=int,
    ...
    parser.add_argument(
        "--beam_width", type=int, default=2, help="beam search width"
    )
    parser.add_argument(
        "--max_depth", type=int, default=2, help="tree depth width"
    )
    parser.add_argument(
        "--w_rel",
        type=float,
    ...
    ret_tokens, rel_tokens, grd_tokens, ut_tokens = load_special_tokens(
        tokenizer,
        use_grounding=args.use_groundness,
    ...
    total_problems = len(input_data)  # Total number of problems
    for i, row in tqdm(enumerate(input_data)):
        ...
        # Print the current status and elapsed time
        elapsed_time = time.time() - start_time
        print(
            f"Processing problem {i+1}/{total_problems} - Elapsed time: {elapsed_time:.2f} seconds"
        )
        ...
        with open(args.output_file + f"_tmp", "w") as outfile:
